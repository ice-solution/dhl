...
const { getFlowRules, CATEGORY_RULE_OVERRIDES, isFlowSectionEnabled } = require('../data/category-flows');
...
function buildCategoryMeta() {
  ...
  const lodgingSectionEnabledByCategory = {};
  ...
  categoryValues.filter(Boolean).forEach((cat) => {
    ...
    lodgingSectionEnabledByCategory[cat] = isFlowSectionEnabled(cat, 'lodging');
  });
  ...
  return {
    ...
    lodgingSectionEnabledByCategory,
    flightCopyByGroup,
    lodgingCopyByGroup,
  };
}
